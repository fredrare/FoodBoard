...
export default function Header() {
    ...
    return <TailwindNavbar
        ...
        className="py-1 bg-red-500 flex flex-wrap w-full"
    >
        <nav>
            <ul className="items-center justify-between text-base lg:flex pt-0">
                {links.map(x => (
                    <li>
                        <Link to={x.link} className="block px-0 py-3 border-b-2 border-transparent lg:p-4 hover:border-indigo-400 transition-all duration-200">
                            {x.name}
                        </Link>
                    </li>
                ))}
